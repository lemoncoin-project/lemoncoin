...
*Setup instructions for a gitian build of LemonCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the LemonCoin
...
- [Building LemonCoin](#building-lemoncoin)
...
Clone the git repositories for lemoncoin and gitian.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the lemoncoin repository
...
To build LemonCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the lemoncoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/lemoncoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/lemoncoin/lemoncoin
...
For example:
```bash
URL=https://github.com/laanwj/lemoncoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit lemoncoin=${COMMIT} --url lemoncoin=${URL} ../lemoncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit lemoncoin=${COMMIT} --url lemoncoin=${URL} ../lemoncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit lemoncoin=${COMMIT} --url lemoncoin=${URL} ../lemoncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/lemoncoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/lemoncoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/lemoncoin-osx-build.assert
```
...
[lemoncoin/gitian.sigs](https://github.com/lemoncoin/gitian.sigs/) repository, or if that's not possible create a pull request. 
